...
import airsim
import threading
class Attitude_Controller_Thread(threading.Thread):
    def __init__():
...
if __name__ == "__main__":
    ...
    client.moveToPositionAsync(1, 0, -7, 5).join()
    # to do
    ...
    while True:
        ...
        print(q1)
        # 计算uf(期望速度（虚）与期望角速度)
        uf, ufd = cal_uf(uf, dt, c1, c2, c3, z_, psi_d, q1, q1_, v)
        psi_d=ufd[3,0]
        print('------------------------uf-----------------')
        print(uf)
